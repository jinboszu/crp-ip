...
class MRIP:
    def __init__(self, bay, bug_fixed=True):
        ...
        self.u = u = model.binary_var_dict(((s, i) for s in irange(1, S - 1) for i in irange(s + 1, S)), name='u')
        self.v = v = model.binary_var_dict(((s, i) for s in irange(1, S - 1) for i in irange(s + 1, S)), name='v')
        ...
        self.z = z = model.binary_var_dict(((s, i) for s in irange(1, S - 1) for i in irange(s + 1, S)), name='z')
        ...
        # (7)
        model.add_constraints(C * u[s, i] >= model.sum(c * x[s, i, c, p] for c in irange(1, C) for p in irange(1, P)) - model.sum(c * x[s, s, c, p] for c in irange(1, C) for p in irange(1, P)) + 1 for s in irange(1, S - 1) for i in irange(s + 1, S))
        # (8)
        model.add_constraints(C * u[s, i] - C <= model.sum(c * x[s, i, c, p] for c in irange(1, C) for p in irange(1, P)) - model.sum(c * x[s, s, c, p] for c in irange(1, C) for p in irange(1, P)) for s in irange(1, S - 1) for i in irange(s + 1, S))
        # (9)
        model.add_constraints(C * v[s, i] >= model.sum(c * x[s, s, c, p] for c in irange(1, C) for p in irange(1, P)) - model.sum(c * x[s, i, c, p] for c in irange(1, C) for p in irange(1, P)) + 1 for s in irange(1, S - 1) for i in irange(s + 1, S))
        # (10)
        model.add_constraints(C * v[s, i] - C <= model.sum(c * x[s, s, c, p] for c in irange(1, C) for p in irange(1, P)) - model.sum(c * x[s, i, c, p] for c in irange(1, C) for p in irange(1, P)) for s in irange(1, S - 1) for i in irange(s + 1, S))
        # (11)
        model.add_constraints(z[s, i] == u[s, i] + v[s, i] - 1 for s in irange(1, S - 1) for i in irange(s + 1, S))
        # (12)
        model.add_constraints(y[s, i] >= z[s, i] - 1 + (model.sum(p * x[s, i, c, p] for c in irange(1, C) for p in irange(1, P)) - model.sum(p * x[s, s, c, p] for c in irange(1, C) for p in irange(1, P))) / P for s in irange(1, S - 1) for i in irange(s + 1, S))
        # (13)
        model.add_constraints(y[s, i] <= z[s, i] for s in irange(1, S - 1) for i in irange(s + 1, S))
        # (14)
        model.add_constraints((model.sum(p * x[s, s, c, p] for c in irange(1, C) for p in irange(1, P)) - model.sum(p * x[s, i, c, p] for c in irange(1, C) for p in irange(1, P))) / P <= 1 - y[s, i] for s in irange(1, S - 1) for i in irange(s + 1, S))
        # (15)
